...
struct ContentView: View {
    ...
    var sample: some View_ {
        HStack_(children: [
            AnyView_(Rectangle_().foregroundColor(NSColor.red).frame(width: 150, height: 50)
                        .alignmentGuide(for: .leading, computeValue: { dimension in
                            dimension.width/2
                        })),
            AnyView_(Rectangle_().foregroundColor(NSColor.green).frame(width: 100, height: 50)
                        .alignmentGuide(for: .leading, computeValue: { dimension in
                            dimension.width/2
                        })),
        ])
